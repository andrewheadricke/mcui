    ...
    return m("div.fixed inset-0 z-2000 " + display,
      ...
      m("aside.absolute left-0 top-0 bottom-0 w-72 bg-gray-950 p-6",
        ...
        m("nav.space-y-3",
          ...
          (()=>{
            // only show Quit button if AndroidBridge is available
            if (globalThis.AndroidBridge != null) {
              return m("button.nav-btn w-full text-left px-4 py-3 rounded-lg bg-primary/10 text-primary font-medium cursor-pointer", {onclick:(e)=>{
                 AppState.toggleMobileSlideOver()
                 globalThis.AndroidBridge.shutdown()
                }},
                m("div.inline-block w-6 me-2", m.trust(svgPlugCircleCheck)), 
                m("div.inline-block align-top", "Quit")
              )
            }
          })()          
        )
